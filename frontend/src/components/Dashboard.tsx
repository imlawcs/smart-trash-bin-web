...
const Dashboard: React.FC = () => {
  ...
  const { trashBins, loading, error } = trashBinState;

  useEffect(() => {
    if (authState.isAuthenticated && !loading && trashBins.length === 0) {
      getTrashBins();
    }
  }, [authState.isAuthenticated, loading, getTrashBins, trashBins.length]);
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            <span className="text-gray-600">Welcome, {authState.user?.name || 'User'}</span>
            <button
              onClick={logout}
