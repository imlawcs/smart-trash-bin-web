...
export const TrashBinProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getCompartmentsByBinId = useCallback(async (binId: string) => {
    const requestKey = `getCompartments_${binId}`;
    if (state.ongoingRequests.has(requestKey)) return;
    ...
    try {
      ...
      dispatch({ type: 'BIN_ERROR', payload: 'Failed to load compartments' });
    } finally {
      dispatch({ type: 'REMOVE_REQUEST', payload: requestKey });
    }
  }, []);
